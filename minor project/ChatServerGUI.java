...
Socket C;
    ...
    public ChatServerGUI() {
		...
		Head.add(header=new JLabel("Chat Server"),BorderLayout.NORTH);
		...
		try{
		Thread.sleep(1000);
		main.append("Welcome to ChatEngine \n");
		main.append("please wait...\n");
		Thread.sleep(2000);		
		main.append("Click StartService to Start Listening on port 27000\n");
		}
		catch(Exception E)
		{}
	...
	{	try {
	JOptionPane.showMessageDialog(null, "Running!", "Ready to accept new clients", JOptionPane.INFORMATION_MESSAGE); 
		serverState=true;
	...
	try
	{
	ChatServerGUI.this.main.append(">Ready to accept clients\n");
	while(true)
	{
	C = s.accept();
	no_of_users++;
	...
	public class threadclient extends Thread
 {
  ...
  threadclient(Socket S)
	...
	public void run()
	{
		...
		pw.println("Welcome to ChatEngine 1.0");
		...
		{	if((msg=br.readLine())!=null)
			...
			if(msg.equals("0x007"))
				{
				pw.println("You have been Disconnected from the Chat");				
				pw.flush();
				com = this.getName();
				...
				{			
					...
					dummy=(Socket)SocketClients.get(i);
							...
							pw=new PrintWriter(dummy.getOutputStream());
							pw.println(ChatServerGUI.this.com+" has left the chat");
							pw.flush();
